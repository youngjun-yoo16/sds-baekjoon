...
def main():
    ...
    # Direction vectors: map direction index to (dr, dc)
    directions = { 0 : (-1, 0), 1 : (-1, 1), 2 : (0, 1), 3 : (1, 1), 
                  4 : (1, 0), 5 : (1, -1), 6 : (0, -1), 7 : (-1, -1) }

    for _ in range(K):
        # Set to record positions where fireballs will combine
        ...
        for r in range(N):
            for c in range(N):
                for _ in range(len(grid[r][c])):
                    ...
                    # Move fireball based on direction and speed, wrap around grid
